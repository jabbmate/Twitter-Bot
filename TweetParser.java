...
public class TweetParser {
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        List<String> dataToTweets = new LinkedList<>();

        while (li.hasNext()) {
            String line = li.next();
            if (tweetColumn < line.split(",").length) {
                dataToTweets.add(extractColumn(line, tweetColumn));
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        List<List<String>> cleanedTweet = new LinkedList<>();
        String tweetNoURL = removeURLs(tweet);
        List<String> tweetSentences = tweetSplit(tweetNoURL);
        for (String sentence: tweetSentences) {
            cleanedTweet.add(parseAndCleanSentence(sentence));
        }
